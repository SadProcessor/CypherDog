...
$ASCII= @("
 ...
 ______||-________...____________________4.1__
 ...
 BloodHound Dog Whisperer - @SadProcessor 2022
")
...
function EdgeString{
    Param(
        [ValidateSet('NoDefault','NoACL','NoGPO','NoSpecial','NoAzure')]
    ...
    Switch -regex ($Filter){
        ...
        NoAzure   {$EdgeList = (Compare $EdgeList ([Enum]::GetNames([EdgeAzr]))).InputObject}
        }
    # Exclude stuff
    ...
    Begin{
        ...
        $WITH = if($WITH){"`r`nWITH $WITH"}else{
            if(($Cypher -AND -Not $Raw) -OR $Return){$Null}Else{
                "`r`nWITH p, LENGTH(p) as ln,`r`n[a in NODES(p)|a.name] as nd,`r`n[b in NODES(p)|LABELS(b)[0]] as lbl,`r`n[c IN RELATIONSHIPS(p)|TYPE(c)] as tp"
                }}
        # Return
    ...
    Begin{
        ...
        $WITH = if($WITH){"`r`nWITH $WITH"}else{
            if(($Cypher -AND -Not $Raw) -OR $Return){$Null}Else{
                ...
                "`r`nWITH p, LENGTH(p) as ln,`r`n[a in NODES(p)|a.$prp] as nd,`r`n[b in NODES(p)|LABELS(b)[0]] as lbl,`r`n[c IN RELATIONSHIPS(p)|TYPE(c)] as tp"
                }}
        # Return
        $RTRN = if($RETURN){"`r`nRETURN $RETURN"}Else{
            if($Cypher -AND -Not$Raw){"`r`nRETURN p"}Else{
